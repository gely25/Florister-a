...
class Bouquet(models.Model):
    user = models.ForeignKey('accounts.User', on_delete=models.SET_NULL, null=True, blank=True, related_name='designed_bouquets')
    size = models.ForeignKey(BouquetSize, on_delete=models.PROTECT)
    created_at = models.DateTimeField(auto_now_add=True)
    total_price = models.DecimalField(max_digits=10, decimal_places=2, default=0.00)

    def __str__(self):
        owner = self.user.get_full_name() if self.user else "Invitado"
        return f"Ramo {self.size.name} ({owner}) - {self.created_at.strftime('%Y-%m-%d %H:%M')}"
